...
class WC_Braspag_Client_Logger
{
	...
	public static function handle_request()
	{
		...
		$raw_entries = isset($_POST['entries']) === TRUE ? wp_unslash($_POST['entries']) : '';
		$entries = json_decode($raw_entries, true);

		if (is_array($entries) === FALSE || empty($entries) === TRUE) {
		...
		foreach ($entries as $entry) {
			$level = isset($entry['level']) === TRUE ? sanitize_text_field($entry['level']) : 'log';
			$text = isset($entry['message']) === TRUE ? sanitize_textarea_field($entry['message']) : '';
